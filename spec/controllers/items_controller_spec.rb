  ...
  context "when logged in" do
    ...
    describe "POST #create" do
      it "increases the number of items by 1" do
        expect { post :create, params: { user_id: user.id, item: { name: FFaker::Name.name } } }.to change(Item, :count).by(1)
      end

      it "assigns the new item to @item" do
        post :create, params: { user_id: user.id, item: { name: FFaker::Name.name } }
